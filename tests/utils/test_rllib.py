import phantom as ph
import pytest

from .. import MockAgent, MockEnv, MockPolicy


def test_rllib_train_rollout(tmpdir):
    ph.utils.rllib.train(
        algorithm="PPO",
        env_class=MockEnv,
        env_config={},
        policies={
            "mock_policy": MockAgent,
        },
        policies_to_train=["mock_policy"],
        tune_config={
            "checkpoint_freq": 1,
            "stop": {
                "training_iteration": 2,
            },
            "local_dir": tmpdir,
        },
        num_workers=1,
    )

    rgen = ph.utils.rllib.RolloutGenerator(directory=f"{tmpdir}/PPO/LATEST")

    # num_repeats < 1
    with pytest.raises(AssertionError):
        list(rgen.rollout(num_repeats=0))

    # num_workers < 0
    with pytest.raises(AssertionError):
        list(rgen.rollout(num_workers=-1))

    # Without workers:
    results = rgen.rollout(num_repeats=3, num_workers=0)
    assert len(list(results)) == 3

    # With workers:
    results = rgen.rollout(num_repeats=3, num_workers=1)
    results = list(results)
    assert len(results) == 3
    assert results[0].actions_for_agent("a1") == [0, 0, 0, 0, 0]

    results = rgen.rollout(
        custom_policy_mapping={"a1": MockPolicy},
        num_repeats=1,
        num_workers=1,
    )
    assert list(results)[0].actions_for_agent("a1") == [1, 1, 1, 1, 1]

    # Evaluate policy:
    results = ph.utils.rllib.PolicyEvaluator(
        directory=f"{tmpdir}/PPO/LATEST",
        obs_space=MockAgent("").observation_space,
        policy_id="mock_policy",
    ).evaluate(0, False)
    assert len(list(results)) == 1


def test_rllib_train_bad(tmpdir):
    # policy to train not defined
    with pytest.raises(ValueError):
        ph.utils.rllib.train(
            algorithm="PPO",
            env_class=MockEnv,
            env_config={},
            policies={
                "mock_policy": MockAgent,
            },
            policies_to_train=["undefined_policy"],
            tune_config={
                "local_dir": tmpdir,
            },
        )
