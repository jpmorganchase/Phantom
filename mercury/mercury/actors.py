...
class SimpleSyncActor(SyncActor):
    ...
    def handle_message(self, ctx: "Network.Context", message: Message) -> Responses:
        ...
        if ptype not in self.__handlers:
            raise KeyError(f"Unknown payload type {ptype} in message sent from '{message.sender_id}' to '{message.receiver_id}'. Agent '{message.receiver_id}' needs a message handler function capable of receiving this mesage type.")
